...
<div class="fade modal inmodal " id="modal-create-user" tabindex="-1" role="dialog" aria-labelledby="modal-create-user-label">
                            ...
                            <div class="ibox-content">
                                <div class="row">
                                    <div class="col-lg-5">
                                        ...
                                        <?= $formCliente->field($model->cliente, 'country_id')->dropDownList(PaisesLatam::getPaises()); ?>
...
<script type="text/javascript">
    ...
    $form_cliente = {
        ...
        $origenCountry: $('#cliente-country_id', $form_cliente_content),
        ...
        $form_esysdireccion.$inputMunicipio.change(function() {
            if ($form_esysdireccion.$inputEstado.val() != 0 && $form_esysdireccion.$inputMunicipio.val() != 0 && $form_esysdireccion.$inputCodigoSearch.val() == "") {
                ...
                $.get('<?= Url::to('@web/municipio/colonia-ajax') ?>', {
                    'estado_id': $form_esysdireccion.$inputEstado.val(),
                    "municipio_id": $form_esysdireccion.$inputMunicipio.val(),
                    'codigo_postal': $form_esysdireccion.$inputColonia.val()
                }, function(json) {
                    if (json.length > 0) {
                        $.each(json, function(key, value) {
                            $form_esysdireccion.$inputColonia.append("<option value='" + value.id + "'>" + value.colonia + "</option>\n");
    ...
    $('#form-cliente').click(function(event) {
       ...
       // if (!$form_cliente.$inputOrigen.val()) {
       //     toastr.options = {
       //         closeButton: true,
       //         progressBar: true,
       ...
       //         timeOut: 5000
       //     };
       //     toastr.warning('El ORIGEN del cliente es requerido, verifica tu información');
       //     return false;
       // }

        if ($form_cliente.$inputOrigen.val() == 2) {
            if (!$form_esysdireccion.$inputMunicipio.val() || !$form_esysdireccion.$inputEstado.val()) {
                toastr.options = {
                    closeButton: true,
                    progressBar: true,
                    showMethod: 'slideDown',
        ...
        $.post('<?= Url::to(['/crm/cliente/cliente-create-ajax']) ?>', $form_cliente_content.serialize(), function($json) {
            if ($json.code != 10) {
                ...
                if (isReceptorCreate) {
                    ...
                    $cliente_receptor.val(false).change();
                    renvio_array = [];
                    clienteSelect = [];
