...
<script>
    ...
    $form_esysdireccion_envio.$inputMunicipio.change(function() {
        if ($form_esysdireccion_envio.$inputEstado.val() != 0 && $form_esysdireccion_envio.$inputMunicipio.val() != 0 && $form_esysdireccion_envio.$inputCodigoSearch.val() == "") {
            ...
            $.get('<?= Url::to('@web/municipio/colonia-ajax') ?>', {
                ...
                'codigo_postal': $form_esysdireccion_envio.$inputColonia.val()
            }, function(json) {
                if (json.length > 0) {
                    $.each(json, function(key, value) {
                        $form_esysdireccion_envio.$inputColonia.append("<option value='" + value.id + "'>" + value.colonia + "</option>\n");
    ...
    $cliente_emisor.change(function() {
        ...
        if ($(this).val() == '' || $(this).val() == null) {
            clear_form($form_emisor);
            clear_form($div_info_emisor.inputText);
            return false;
        }
        ...
        $.get('<?= Url::to('@web/municipio/municipios-ajax') ?>', {
            'estado_id': clienteEmisor[key].estado_id
        }, function(json) {
            municipioList = json;
            ...
            for (var i = 0; i < $(this).val().length; i++) {
                $.get('<?= Url::to(['cliente-info-ajax']) ?>', {
                    ...
                    if (json) {
                        ...
                        $.get("<?= Url::to(['cliente-zona']) ?>", {
                            cliente_id: json.id,
                            code: json.codigo_postal
                        }, function($response) {
                            console.log('ascsadvS',$response);
                            if($response.is_zona_roja){
                                $("#msg_error").show();
                                
                            }else{
                                $("#msg_error").hide();
                            }
                            $("#input_zona_roja").val($response.is_zona_roja);
                        }, 'json');
                        ...
                        $.get('<?= Url::to('@web/municipio/municipios-ajax') ?>', {
                            ...
                            reenvio = {
                                ...
                                "estado_id": json.estado_id,
                                "estado_text": estadoList[json.estado_id],
                                "municipio_id": json.municipio_id,
                                "municipio_text": municipioList[json.municipio_id],
                                "colonia_id": json.codigo_postal_id,
                                "colonia_text": json.colonia,
                                "direccion": json.direccion,
                                "n_interior": json.num_int,
                                "n_exterior": json.num_ext,
    ...
    $(".modal-create").click(function() {
        ...
        if ($.trim($(this).data("cliente")) == 'Emisor') {
            ...
            if ($cliente_emisor.val()) {
                $.get("<?= Url::to(['/crm/cliente/cliente-ajax'])  ?>", {
                    cliente_id: $cliente_emisor.val()
                }, function(cliente_json) {
                    if (cliente_json) {
            ...
            if ($cliente_receptor.val().length > 0 && $is_action == 'Update') {
                ...
                $.get("<?= Url::to(['/crm/cliente/cliente-ajax'])  ?>", {
                    ...
                    if (cliente_json) {
                        ...
                        $form_cliente.$inputOrigen.val(cliente_json.results.origen).trigger('change');
                        $form_cliente.$telefono.val(cliente_json.results.telefono);
                        $form_cliente.$origenCountry.val(cliente_json.results.country_id);
                        $form_cliente.$telefono_movil.val(cliente_json.results.telefono_movil);
                        $form_esysdireccion.$inputDireccion.val(cliente_json.results.direccion);
                        $form_esysdireccion.$inputNumeroExt.val(cliente_json.results.num_ext);
                        $form_esysdireccion.$inputNumeroInt.val(cliente_json.results.num_int);
