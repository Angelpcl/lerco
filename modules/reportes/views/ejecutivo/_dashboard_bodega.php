...
<script>
	...
	var load_ini = function(){
		...
		$.get("<?= Url::to(['reporte-data-bodega'])  ?>",{ filters: $filterArray.serialize() },function($response){
			if ($response) {

				$('.lbl_venta_money').html( ($response.venta.total_venta ? btf.conta.money(parseFloat($response.venta.total_venta)) : 0));
				$('.lbl_venta_lb').html(($response.venta.lb_venta ? parseFloat($response.venta.lb_venta).toFixed(2) : 0));
				$('.lbl_venta_pz').html(($response.venta.pz_venta ? $response.venta.pz_venta :0 ));

				$.get("<?= Url::to(['trailer-reporte'])  ?>",{ filters: $filterArray.serialize() },function($response2){
					if ($response2) {
						$.each($response2.trailer,function(key,item){
							if (array_item.length == 0 )
								$item = {
									name: item.id + "- ["+ item.nombre_chofer + "- " + btf.conta.money(parseFloat(item.total)) +"]",
