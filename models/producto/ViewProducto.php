...
class ViewProducto extends \yii\db\ActiveRecord
{
    ...
    public static function getProductoSeachAjax($q,$lax_tierra = false,$mex = false)
    {
        ...
        $query = (new Query())
            ->select([
                ...
                END AS `text`",
                "view_producto.categoria_id",
                'view_producto.tipo_servicio',
                'view_producto.nota',
                'view_producto.status',
                'view_producto.is_impuesto',
                'view_producto.costo_extra',
                'esys_lista_desplegable.is_mex',
                'esys_lista_desplegable.mex_costo_extra',
                'esys_lista_desplegable.mex_intervalo',
                'esys_lista_desplegable.mex_required_min',
                'view_producto.required_min',
                'view_producto.impuesto_old',
                'view_producto.intervalo',
                'view_producto.unidad_medida',
                'view_producto.categoria',
                'view_producto.created_at',
                'view_producto.created_by',
                'view_producto.created_by_user',
                'view_producto.updated_at',
                'view_producto.updated_by',
                'view_producto.updated_by_user',

            ])
            ->from(self::tableName())
            ->leftJoin('esys_lista_desplegable', 'esys_lista_desplegable.id = view_producto.categoria_id')
            ->andWhere([ 'is_producto' =>  Producto::TIPO_PRODUCTO ])
            ->orderBy('id desc')
            ->limit(50);
    ...
    public static function getProductoAllJsonBtt($arr)
    {
     ...
     public static function getProductoMexAjax($producto_id, $sucursal_receptor_id, $destino_id)
    {
        // La respuesta sera en Formato JSON
        Yii::$app->response->format = Response::FORMAT_JSON;
        ...
        $producto = (new Query())
            ...
            ->from(self::tableName())
            ->leftJoin('esys_lista_desplegable', 'esys_lista_desplegable.id = view_producto.categoria_id')
            ->andWhere(['view_producto.id' => $producto_id ])->one();
        ...
        if (isset($producto["id"])) {
            ...
            ->andWhere(["and",
                ...
                ['=','tipo', ListaPrecioMx::TIPO_IMPUESTO ],
            ])
            ->one();

            if (!isset($listaCostoExtra["id"])) {
                $listaCostoExtra   =  (new Query())
                ->select([
                    "`lista_precio_mx`.`id`",
                    "`intervalo`",
            ...
            if (isset($listaCostoExtra["id"])) {
                $producto["is_mex"] = 10;
                $producto["mex_required_min"]   = $listaCostoExtra["required"];
                $producto["mex_intervalo"]      = $listaCostoExtra["intervalo"];
                $producto["mex_costo_extra"]    = $listaCostoExtra["impuesto"];
            }

        }
        return isset($producto["id"]) ? $producto : [];
    }
